...
class Iphdr:
    def __init__(self, tot_len, protocol, saddr, daddr):
        self.ver_len = 0x45  # IPv4 + header length(5*4 = 20 bytes)
        self.tos = 0
        ...
        self.check = 0
        self.saddr = socket.inet_aton(saddr)  # convert to 4-byte format
...
def unpack_Iphdr(buffer):
    return struct.unpack('!BBHHHBBH4s4s', buffer[:20])

def getPacketSize(unpacked_ipheader):
    return unpacked_ipheader[2]  # total length

def getProtocolId(unpacked_ipheader):
    return unpacked_ipheader[6]  # protocol number
...
if __name__ == '__main__':
    ...
    print("Packed IP Header:", binascii.b2a_hex(packed_iphdr))

    unpacked_iphdr = unpack_Iphdr(packed_iphdr)
    print("Unpacked Header:", unpacked_iphdr)

    print(
        'Packet size: {} | Protocol: {} | Src/Dst IP: {}'
        .format(
            getPacketSize(unpacked_iphdr),
            getProtocolId(unpacked_iphdr),
            getIP(unpacked_iphdr)
        )
    )
